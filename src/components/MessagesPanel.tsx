...
export const MessagesPanel = ({ currentUserId, targetUserId, isAdmin }: Props) => {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className={`flex ${isAdmin ? "flex-col sm:flex-row" : ""} gap-4 h-[500px]`}>
          {isAdmin && (
            <div className="sm:w-56 w-full border-b sm:border-b-0 sm:border-r border-border pb-3 sm:pb-0 sm:pr-4 overflow-y-auto shrink-0 max-h-[120px] sm:max-h-none">
              {conversations.length === 0 ? (
                <p className="text-xs text-muted-foreground">Sem conversas</p>
              ) : (
                <div className="flex sm:flex-col gap-2 sm:gap-1 overflow-x-auto sm:overflow-x-visible">
                  {conversations.map((c) => (
                    <button
                      key={c.userId}
                      onClick={() => setSelectedUser(c.userId)}
                      className={`text-left p-2 rounded text-sm hover:bg-muted/50 transition-colors shrink-0 sm:w-full ${selectedUser === c.userId ? "bg-muted" : ""}`}
                    >
              ...
              <>
                  ...
                  <div className="space-y-3 p-2">
                    ...
                    {messages.map((m) => (
                      <div key={m.id} className={`flex ${m.sender_id === currentUserId ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm break-words ${m.sender_id === currentUserId ? "bg-primary text-primary-foreground rounded-br-md" : "bg-muted text-foreground rounded-bl-md"}`}>
                ...
                <div className="flex gap-2 mt-3">
                  <Textarea
                    ...
                    rows={2}
                  />
                  <Button onClick={sendMessage} disabled={!input.trim() || sending} size="icon" className="shrink-0 self-end h-10 w-10">
